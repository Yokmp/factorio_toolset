...
def mod_list_warning(mod_list_file: Path) -> str | None:
    """Return a user-facing warning when Factorio's mod-list.json has no useful mod state."""
    if not mod_list_file.exists():
        return (
            f"Factorio mod-list.json was not found:\n{mod_list_file}\n\n"
            "Start Factorio once and wait until the main menu is visible, then refresh this tool."
        )

    entries = read_mod_list_entries(mod_list_file)
    if not entries:
        return (
            f"Factorio mod-list.json has no readable mod entries:\n{mod_list_file}\n\n"
            "Start Factorio once and wait until the main menu is visible, then refresh this tool."
        )

    enabled = {entry["name"] for entry in entries if entry.get("enabled")}
    listed = {entry["name"] for entry in entries}
    if listed <= set(VANILLA_MODS) or enabled <= set(VANILLA_MODS):
        return (
            f"Factorio mod-list.json only contains the base game:\n{mod_list_file}\n\n"
            "If you expect installed mods here, start Factorio once and wait until the main menu is visible. "
            "Factorio usually writes the full mod list after it has scanned the mods folder."
        )
