...
```powershell
python F:\Path_to\Factorio\mods\Toolsets\factorio-toolset\ui.py
```

Run a mod's harness directly:

```powershell
python F:\Path_to\Factorio\mods\Toolsets\testharness\run_tests.py --mod-root F:\Path_to\Factorio\mods\MOD_NAME --profile default --no-color
```
